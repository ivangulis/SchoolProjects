...
public class MainWindowController {
	...
	public Label getLabelAllChamps() {
		return labelAllChamps;
	}
	public User getUser() {
		return user;
	}
	
	public Label getLabelPrint() {
		return labelPrint;
	}
	...
	public void setUser(User user) {
		...
		labelAllChamps.getStyleClass().add("roles");
	...
	public void refreshChampionsFunction() {
		...
		try {
			...
			LOG.info("Connection to ChampionsFacadeBean initialized");
			...
			LOG.info("All champions were successfully loaded from database.");
			String text = Main.getInstance().getLanguage().getString("MainWindowLoad");
			labelPrint.setText(text);
			...
			String text = Main.getInstance().getLanguage().getString("MainWindowContextError");
			labelPrint.setText(text);
			...
			String text = Main.getInstance().getLanguage().getString("MainWindowLoadError");
			labelPrint.setText(text);
		} finally {
			try {
				ctx.close();
	...
	public void adminControlActualize(ActionEvent event) {
		...
		try {
			...
			LOG.info("All champions from web page were successfully saved into database.");
		} catch (NamingException e) {
			LOG.log(Level.SEVERE, "Context was not able to be initialized - name resolution has failed.", e);
	...
	public void savePool(ActionEvent event) {
		...
		String text = Main.getInstance().getLanguage().getString("MainWindowSave");
		labelPrint.setText(text);
		} catch (NamingException e) { //logging from this function
			LOG.log(Level.SEVERE, "Context was not able to be initialized - name resolution has failed.", e);
			String text = Main.getInstance().getLanguage().getString("MainWindowContextError");
			...
			String text = Main.getInstance().getLanguage().getString("MainWindowSaveError");
			labelPrint.setText(text);
		} finally {
			try {
				ctx.close();
	...
	public void loadPoolFunction() {
		...
		LOG.info("Pool tables were successfully filled with user's champions.");
	}
	...
	public void removeFromPool(TableView<TableChampion> table, TableChampion row) {
		if (hashSetChampions.remove(row.getChampion().getName())) {
			...
			LOG.info("Champion was successfully removed from pool table.");
		}
	}
	...
	public void addToPool() {
		...
		if (!hashSetChampions.contains(champion.getName())) {
			...
			case "Top Lane": 
				tableTops.getItems().add(row);
				LOG.info("Champion was successfully added into TOP pool table.");
				break;
			case "Jungler": 
				tableJungles.getItems().add(row);
				LOG.info("Champion was successfully added into JUNGLE pool table.");
				break;
			case "Middle Lane": 
				tableMids.getItems().add(row);
				LOG.info("Champion was successfully added into MID pool table.");
				break;
			case "AD Carry":
				tableAdcs.getItems().add(row);
				LOG.info("Champion was successfully added into ADC pool table.");
				break;
			case "Support":
				tableSupports.getItems().add(row);
				LOG.info("Champion was successfully added into SUPPORT pool table.");
				break;
			default:
				LOG.warning("Champion was not added into pool table. Incorrect role.");
			}
			String text = Main.getInstance().getLanguage().getString("MainWindowAdd");
			labelPrint.setText(text);
		} else {
			String text = Main.getInstance().getLanguage().getString("MainWindowAddError");
			labelPrint.setText(text);
		}
	}
	...
	private void loadBanList() {
		...
		try {
			...
			LOG.info("For banlist, 6 most banned champions were successfully loaded from database.");
		} catch (NamingException e) { //logging from this function
			LOG.log(Level.SEVERE, "Context was not able to be initialized - name resolution has failed.", e);
		} catch (RuntimeException e) { //logging from getTopBans
			LOG.warning("Banlist was not loaded from database into banList tabe.");
		} finally {
			try {
				ctx.close();
			} catch (NamingException e) {
	...
	private void showDetails(Champion champion) {
		...
		TableDetails role = new TableDetails(Main.getInstance().getLanguage()
				.getString("Role") + ":", champion.getRole());
		tableDetails.getItems().add(role);
		TableDetails winRatio = new TableDetails(Main.getInstance().getLanguage()
				.getString("Win") + ":", champion.getWinRatio().toString() + "%");
		tableDetails.getItems().add(winRatio);
		TableDetails playRatio = new TableDetails(Main.getInstance().getLanguage()
				.getString("Play") + ":", champion.getPlayRatio().toString() + "%");
		tableDetails.getItems().add(playRatio);
		TableDetails banRatio = new TableDetails(Main.getInstance().getLanguage()
				.getString("Ban") + ":", champion.getBanRatio().toString() + "%");
		tableDetails.getItems().add(banRatio);
		TableDetails ipCost = new TableDetails(Main.getInstance().getLanguage()
				.getString("IPCost"), champion.getIpCost().toString() + " IP");
		tableDetails.getItems().add(ipCost);
		TableDetails rpCost = new TableDetails(Main.getInstance().getLanguage()
				.getString("RPCost"), champion.getRpCost().toString() + " RP");
		tableDetails.getItems().add(rpCost);
		TableDetails released = new TableDetails(Main.getInstance().getLanguage()
				.getString("Released"), champion.getReleased());
	...
	public void logOut(ActionEvent event) {
		...
		LOG.info("User was successfully logged out, main window was hid, login window shown.");
	}
